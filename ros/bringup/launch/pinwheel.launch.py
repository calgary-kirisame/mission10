"""Pinwheel intelligent-flight bringup — N drones in one gz world.

Launches the PX4 SITL fleet (num_vehicles), one gz-truth EV bridge per drone
(namespaced odometry so they don't collide), and one pinwheel_mission node per
drone. Each drone flies the identical launch-relative orbit, differing only by
phase; spawn offsets (fleet.yaml, 3 m apart) reconstruct the world pinwheel.
...
def _setup(context, *args, **kwargs):
    ...
    if not mission_config:
        mission_config = os.path.join(
            get_package_share_directory("flight_intelligent"), "config", "pinwheel.yaml")
    ...
    for i in range(num):
        ...
        mission_nodes.append(Node(
            ...
            executable="pinwheel_mission",
            name=f"pinwheel_mission_{i}",
            output="screen",
